    ...
    <div>
        ...
        <div class="container">
            ...
            <el-table :data="tableData" border style="font-size:10px" stripe class="table" ref="multipleTable" header-cell-class-name="table-header"  @cell-click="showSOInfo" @selection-change="handleSelectionChange" height="63vh">
                ...
                <el-table-column prop="salesName" width="70" label="销售员"></el-table-column>
                <!-- <el-table-column prop="BOMName" width="90" label="模板名称" align="center"></el-table-column> -->
        ...
        <el-dialog title="" :visible.sync="editVisible" width="35%">        
            <el-form id = "turnoverFunform" :model="turnoverFunform" label-width="170px" :rules="rules" ref="turnoverFunform">
                <el-form-item label="商品往返：" prop="variateName">
                    <el-select v-model="turnoverFunform.variateName"  style="width:15vw;" placeholder="请选择" @change="checkQty">
                        <el-option v-for="item in outOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="数量：" prop="variateQty">
                    <el-input-number v-model="turnoverFunform.variateQty" class="num_input" :precision="3" :controls="false" placeholder="请输入数量" @blur = "checkQty"></el-input-number>
                    <!-- <input v-model="turnoverFunform.variateQty" class="num_input" oninput="value=value.replace(/[^\d.]/g, '').replace(/\.{2,}/g, '.').replace('.', '$#$').replace(/\./g, '').replace('$#$', '.').replace(/^(\-)*(\d+)\.(\d\d).*$/, '$1$2.$3').replace(/^\./g, '')"  @blur = "checkQty"> -->
                </el-form-item>
        ...
        <!-- 绑定BOM -->
        <el-dialog title="" :visible.sync="editTemVisible" width="35%">
            <el-form ref="BOMform" :model="BOMform" label-width="170px">
                <el-form-item label="选择生产BOM模板：" id="bom">
                    <el-cascader v-model="BOMId" :options="options" :props="{ expandTrigger: 'hover' }"></el-cascader>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="editTemVisible = false">取 消</el-button>
                <el-button type="primary" @click="bindBOM">确 定</el-button>
            </span>
        </el-dialog>
        ...
        <el-dialog title="" :visible.sync="turnoverVisible" width="55%">
            <el-tabs v-model="activeName" @tab-click="handleClick" style="padding:0 10px;">
                ...
                <el-tab-pane label="增加商品" name="add"></el-tab-pane>
                <el-tab-pane label="减少商品" name="subtract"></el-tab-pane>
                <el-tab-pane label="报费商品" name="cancellation"></el-tab-pane>
                <el-tab-pane label="其他" name="other"></el-tab-pane>
            </el-tabs>
            <el-table :data="turnoverListData" style="font-size:10px" class="table" ref="multipleTable" height="58vh" header-cell-class-name="table-header">
                <el-table-column prop="number" width="100" :show-overflow-tooltip="true" label="物料编号"></el-table-column>
                <el-table-column prop="name" label="物料名称" ></el-table-column>
                <el-table-column prop="variateName" v-if="activeName=='all'" width="100" :show-overflow-tooltip="true" label="往返项目">
                    <template slot-scope="scope">
                        {{scope.row.variateName | formatTurnover}}
                    </template>
                </el-table-column>
                <el-table-column prop="variateQty" label="数量" width="80" align="center"></el-table-column>                                             
                <el-table-column prop="unitName" width="50" label="单位" align="center"></el-table-column>  
                <el-table-column prop="WHCreateTime" width="180" label="操作时间" :show-overflow-tooltip="true"></el-table-column>                       
            </el-table>

            <div class="pagination">
                <el-pagination background layout="total, prev, pager, next" :current-page="TPageIndex" :page-size="TPageSize" :total="TPageTotal" @current-change="turnoverHandlePageChange"></el-pagination>
            </div>
        </el-dialog>
...
export default {
    ...
    methods: {
        ...
        checkQty(){
            const self = this;
            if(self.turnoverFunform.variateQty && self.turnoverFunform.variateName && self.turnoverFunform.variateName!='add'){
                if(self.turnoverFunform.variateQty>self.turnoverFunform.qty){
                    self.$message.warning('超出需求数量');
                }
            }
        ...
        saveEdit() {
            const self = this;
            let upQty = 0;
            self.$refs.turnoverFunform.validate(valid => {
                if (valid) {
                    if(self.turnoverFunform.variateName!='add'){
                        upQty = self.turnoverFunform.qty*1 - self.turnoverFunform.variateQty*1;
                    }else{
                        upQty = self.turnoverFunform.qty*1 + self.turnoverFunform.variateQty*1;
                    }
                    if(upQty<0){
                    ...
                    turnoverLog({
                        'WHgoodsID':self.turnoverFunform.invId,
                        'BillID':self.turnoverFunform.BillID,
                        'variateName':self.turnoverFunform.variateName,
                        'variateQty':self.turnoverFunform.variateQty,
                        'upQty':upQty,
                        'WHBillNo':self.randomNumber('WH'),
                        'WHApplyName':sessionStorage.getItem('admin_name')
                    }).then(res => {
                        if(res.code == "00") {
                            self.showSOInfo(self.showInfoform,self.column);
                        }else{
                    ...
                    this.$message.error('请选择往返事项和数量');
                    return false;
                }
            });
           self.turnoverFunform = {};            
        },
        ...
        bindBOM() {
            const self = this;
            self.editTemVisible = false;
            if(self.BOMId<1){
                self.$message.error('请选择BOM模板');
                return false;
            }
            ...
            eidtOrderBOMId({
                    'id':self.BOMform.id,
                    'BOMId':self.BOMId[self.BOMId.length-1]//确认生产绑定BOM
                }).then(res => {
                    if(res.code == "00"){               
                        self.getData();     
                        self.$message.success('BOM模板绑定成功');
                    }else  {
                        self.$message.error('BOM模板绑定失败');
                        return false;
                    }
                }).catch(function(err) {
                    self.$message.error('数据处理失败，请检查数据是否正确。');                            
            ...
            eidtOrderState({
                ...
                'billStatus':4//确认生产绑定BOM
            }).then(res => {
                if(res.code != "00"){
                    self.$message.error('订单状态修改失败');
                    return false;
                }
            }).catch(function(err) {
                self.$message.error('数据处理失败，请检查数据是否正确。');                            
            ...
            //通过bomid在ci_bom表中拿到BOM数据
            seaBomAllOneData({
                'bomId':self.BOMId[self.BOMId.length-1],
                'BillID':self.BOMform.id
            }).then(res => {
                if(res.code != "00") {
                    self.$message.error('BOM模板数据导入失败');
                    return false;
                }
            }).catch(function(err) {
                self.$message.error('数据处理失败，请检查数据是否正确。');                            
