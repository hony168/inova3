    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 计量单位管理
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                <el-button v-if="userPower.indexOf('11001')>-1" type="danger" icon="el-icon-delete" class="handle-del mr10" @click="delAllSelection">批量删除</el-button>
                <el-input v-model="q" placeholder="关键字" class="handle-input mr10"></el-input>
                <el-button type="primary" icon="el-icon-search" @click="handleSearch">搜索</el-button>
                <el-button v-if="userPower.indexOf('11000')>-1" type="success" icon="el-icon-plus" @click="handleADD">增加计量单位</el-button>
            </div>
            <el-table :data="tableData" border style="font-size:10px" stripe class="table" ref="multipleTable" header-cell-class-name="table-header" :show-header='true' @selection-change="handleSelectionChange" height="63vh">
                ...
                <el-table-column prop="name" label="计量单位名称" :show-overflow-tooltip="true"></el-table-column>
                ...
                <el-table-column label="操作" width="60" align="center" v-if="userPower.indexOf('11002')>-1" >
                    <template slot-scope="scope">
        ...
        <!-- 编辑弹出框 -->
        <el-dialog title="编辑计量单位" align='center' :before-close="partClose" :visible.sync="editVisible" width="30vw">
            <el-form ref="editForm" id="editForm" :model="editForm" :rules="rules" label-width="100px" class="demo-ruleForm">
                <el-row style="border:1px solid #000;text-align:left;">
                    <div class="tab_title"><i class="el-icon-bank-card"></i> 计量单位信息</div><el-row style="border:1px solid #000;margin:2px;">
                        <el-col class="list_unit b_b" >
                            <el-form-item label="名称：" prop="name">
                                <el-input v-model="editForm.name" placeholder="请输入计量单位名称"></el-input>
                            </el-form-item>
                        </el-col>  
                    </el-row>
                </el-row>
        ...
        <!-- 增加物料弹出框 -->
        <el-dialog title="计量单位增加" align='center' :before-close="partClose" :visible.sync="addVisible" width="30vw">
            <el-form :model="ADDform" :rules="rules" id = "ADDform" ref="ADDform" label-width="100px" class="demo-ruleForm">

                <el-row style="border:1px solid #000;text-align:left;">
                    <div class="tab_title"><i class="el-icon-bank-card"></i> 计量单位信息</div>
                    <el-row style="border:1px solid #000;margin:2px;">
                        <el-col class="list_unit b_b" >
                            <el-form-item label="名称：" prop="name">
                                <el-input v-model="ADDform.name" placeholder="请输入计量单位名称"></el-input>
                            </el-form-item>
                        </el-col>  
                    </el-row>
                </el-row>
...
<script>
import { unitAllList,addUnit,editUnit,delUnit} from '../../api/subsidiary';
export default {
    name: 'Cselect',
    data() {
        return {
            ...
            rules: {
                name: [
                    { required: true, message: '请输入计量单位编号', trigger: 'blur' }
                ]
            }
        };
    },
    ...
    methods: {
        ...
        saveEdit(formName) {
            ...
            self.$refs[formName].validate((valid) => {
                if (valid) {
                    editUnit(self.editForm).then(res => {
                        if(res.code == "00"){
                            self.getData();
