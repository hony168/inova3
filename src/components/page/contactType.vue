    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 供应商类别管理
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                <el-button v-if="userPower.indexOf('6001')>-1" type="danger" icon="el-icon-delete" class="handle-del mr10" @click="delAllSelection">批量删除</el-button>
                <el-input v-model="q" placeholder="关键字" class="handle-input mr10"></el-input>
                <el-button type="primary" icon="el-icon-search" @click="handleSearch">搜索</el-button>
                <el-button v-if="userPower.indexOf('6000')>-1" type="success" icon="el-icon-plus" @click="handleADD">增加供应商类别</el-button>
            </div>
            <el-table :data="tableData" border style="font-size:10px" stripe class="table" ref="multipleTable" header-cell-class-name="table-header" :show-header='true' @selection-change="handleSelectionChange" height="63vh">
                ...
                <el-table-column prop="name" label="供应商类别名称" :show-overflow-tooltip="true"></el-table-column>
                <el-table-column prop="status" label="状态" align="center" width="70" >
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.status==0?'success':(scope.row.status==0?'danger':'danger')">{{scope.row.status===0?'正常':'锁定'}}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column label="操作" width="60" align="center" v-if="userPower.indexOf('6002')>-1" >
                    <template slot-scope="scope">
        ...
        <!-- 编辑弹出框 -->
        <el-dialog title="编辑供应商类别" align='center' :before-close="partClose" :visible.sync="editVisible" width="30vw">
            <el-form ref="editForm" id="editForm" :model="editForm" :rules="rules" label-width="100px" class="demo-ruleForm">
                <el-row style="border:1px solid #000;text-align:left;">
                    <div class="tab_title"><i class="el-icon-bank-card"></i> 供应商类别信息</div><el-row style="border:1px solid #000;margin:2px;">
                        <el-col class="list_unit b_b" >
                            <el-form-item label="名称：" prop="name">
                                <el-input v-model="editForm.name" placeholder="请输入供应商类别名称"></el-input>
                            </el-form-item>
                        </el-col>  
                    </el-row>
                </el-row>
        ...
        <!-- 增加物料弹出框 -->
        <el-dialog title="供应商类别增加" align='center' :before-close="partClose" :visible.sync="addVisible" width="30vw">
            <el-form :model="ADDform" :rules="rules" id = "ADDform" ref="ADDform" label-width="100px" class="demo-ruleForm">

                <el-row style="border:1px solid #000;text-align:left;">
                    <div class="tab_title"><i class="el-icon-bank-card"></i> 供应商类别信息</div>
                    <el-row style="border:1px solid #000;margin:2px;">
                        <el-col class="list_unit b_b" >
                            <el-form-item label="名称：" prop="name">
                                <el-input v-model="ADDform.name" placeholder="请输入供应商类别名称"></el-input>
                            </el-form-item>
                        </el-col>  
                    </el-row>
                </el-row>
...
<script>
import { clientOrContactList,addClientOrContact,editClientOrContact,delClientOrContact} from '../../api/subsidiary';
export default {
    name: 'Cselect',
    data() {
        return {
            ...
            rules: {
                name: [
                    { required: true, message: '请输入供应商类别名称', trigger: 'blur' }
                ]
            }
        };
    },
    ...
    methods: {
        ...
        addContact(formName) {
            ...
            self.$refs[formName].validate((valid) => {
                if (valid) {
                    self.ADDform.typeNumber = 'supplytype';
        ...
        saveEdit(formName) {
            ...
            self.$refs[formName].validate((valid) => {
                if (valid) {
                    self.editForm.typeNumber='supplytype';
